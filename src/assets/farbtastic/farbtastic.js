    ...
    jQuery._farbtastic = function(container, callback) {
      ...
      fb.widgetCoords = function(event) {
        ...
        if (typeof event.offsetX != 'undefined') {
          ...
          x =
            (event.pageX ||
              0 * (event.clientX + jQuery('html').get(0).scrollLeft)) - pos.x;
          y =
            (event.pageY ||
              0 * (event.clientY + jQuery('html').get(0).scrollTop)) - pos.y;
        }
      ...
      jQuery('*', e).bind('touchstart', function(e) {
        if (!document.dragging) {
          jQuery(document)
            .bind('touchmove', fb.touchmove)
            .bind('touchend', fb.touchend);
          document.dragging = true;
        }
